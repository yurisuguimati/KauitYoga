...
namespace KaiutYoga.Controllers
{
    ...
    public class StudentController : Controller
    {
        ...
        {   
            string fileName = @"C:\Users\KaiutYogaBoqueirao\Documents\Relatorio.xlsx";
            ...
            using (SpreadsheetDocument document = SpreadsheetDocument.Create(fileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                List<DateTime> lastPresence = new List<DateTime>();

                UInt32Value index = 1;
                Row row = new Row() { RowIndex = index };
                row.Append(CreateCell("A", index, "Aluno"));
                row.Append(CreateCell("B", index, "Idade"));
                row.Append(CreateCell("C", index, "Plano"));
                row.Append(CreateCell("D", index, "Aulas experimentais"));
                row.Append(CreateCell("E", index, "Ativo"));
                row.Append(CreateCell("F", index, "Data de cadastro"));
                row.Append(CreateCell("G", index, "Data de matrícula"));
                row.Append(CreateCell("H", index, "Inativo - Data da última aula"));
                ...
                foreach (StudentModel item in students)
                {
                    ...
                    row.Append(CreateCell("A",index, item.Name));
                    row.Append(CreateCell("B", index, item.Age));
                    row.Append(CreateCell("C", index, item.Plan.Name));
                    row.Append(CreateCell("D", index, item.TrialClassesPresent()));
                    if (item.Active)
                        row.Append(CreateCell("E", index, "Sim"));
                    else
                        row.Append(CreateCell("E", index, "Não"));
                    row.Append(CreateCell("F", index, item.RegisterDate.ToShortDateString()));

                    if (item.WeeklyPresence != null && item.WeeklyPresence.Count > 0)
                        row.Append(CreateCell("G", index, item.WeeklyPresence.OrderBy(c => c.Date).First().Date.ToShortDateString()));

                    if (!item.Active)
                    {
                        if (item.WeeklyPresence != null && item.WeeklyPresence.Count > 0)
                            lastPresence.Add(item.WeeklyPresence.OrderByDescending(c => c.Date).First().Date);
                        if (item.TrialPresence != null && item.TrialPresence.Count > 0)
                            lastPresence.Add(item.TrialPresence.OrderByDescending(c => c.Date).First().Date);
                        if (item.ReplacementPresence != null && item.ReplacementPresence.Count > 0)
                            lastPresence.Add(item.ReplacementPresence.OrderByDescending(c => c.Date).First().Date);

                        if (lastPresence.Count > 0)
                            row.Append(CreateCell("H", index, lastPresence.OrderByDescending(c => c.ToShortDateString()).First().ToShortDateString()));
                    }
        ...
        public Cell CreateCell(string column, UInt32Value row, object value, bool bold = false)
        {
